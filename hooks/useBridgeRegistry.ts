...
import { AddressTranslator, BridgeRPCHandler as ForceBridgeRPCHandler } from 'nervos-godwoken-integration';
...
export const useBridgeRegistry = ({
  ...
  useEffect(() => {
    async function initBridges() {
      ...
      const httpPolyjuiceProvider = new PolyjuiceHttpProvider(config.godwokenRpcUrl, providerConfig);
      const web3PolyjuiceProvider = new providers.Web3Provider(httpPolyjuiceProvider);

      const web3CKBProvider = new Web3ModalProvider(web3);

      const indexerCollector = new IndexerCollector(config.ckbIndexerUrl);
      const pwCoreClient = await new PWCore(config.ckbRpcUrl).init(web3CKBProvider, indexerCollector);

      const forceBridgeClient = new ForceBridgeRPCHandler(config.bridge.forceBridge.url);
      ...
      const ckbNetwork = new CkbNetwork(network, 'ckb', 'CKB', web3, indexerCollector, pwCoreClient, addressTranslator);

      const ethereumNetwork = new EthereumNetwork(network, 'ethereum', 'Ethereum', provider);
