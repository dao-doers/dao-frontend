...
import { IDisplayValue, AccountBoundToken, BridgeFeature, IBridge } from 'interfaces/data';
import { getDisplayValue } from 'components/Input/BigNumberInput.utils';
...
export const useBridge = ({
  ...
  useEffect(() => {
    ...
    const fetchTokens = async (bridge: IBridge, provider: providers.JsonRpcProvider): Promise<void> => {
      const network = bridge.getDepositNetwork();
      const tokensRegistry = network.getTokens();

      if (selectedFeature === BridgeFeature.Deposit) {
        const tokensSymbols = Object.keys(tokensRegistry) as CanonicalTokenSymbol[];
        ...
        const accountBoundTokens: AccountBoundToken[] = await Promise.all(
          tokensSymbols.map(async tokenSymbol => {
            ...
            const balance = await network.getBalance(token.address, accountAddress);
            return {
              ...token,
              balance,
            };
  ...
  }, [initialized, provider, polyjuiceProvider, selectedFeature, godwokenBridge, setTokens]);
